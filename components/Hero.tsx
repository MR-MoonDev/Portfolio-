...
import { ArrowDown, Cpu, Sparkles, Download, FileText } from 'lucide-react';
import myimage from '../assets/myimage.png'; // Replace with actual image path
...
const Hero: React.FC = () => {
  ...
  const handleDownloadResume = (e: React.MouseEvent<HTMLAnchorElement>) => {
    // For this demo, we prevent default navigation and create a dummy file to download
    // In a real app, this would just point to your /resume.pdf in the public folder
    e.preventDefault();

    const element = document.createElement("a");
    const file = new Blob(
      [
        `ALEX STERLING - RESUME\n\nFull Stack Developer specializing in React, TypeScript, and Node.js.\n\nExperience:\n- Senior Frontend Engineer at TechCorp\n- UI/UX Designer at CreativeStudio\n\nSkills:\n- React, Tailwind, Node.js, TypeScript, PostgreSQL\n\nContact: alex@example.com`
      ],
      { type: 'text/plain' }
    );
    element.href = URL.createObjectURL(file);
    element.download = "Alex_Sterling_Resume.txt";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row justify-center md:justify-start gap-4">
              {/* Added Resume Button */}
              <a
                href="/Muneeb.pdf" // Replace with actual resume path
                download
